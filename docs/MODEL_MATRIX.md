...
| Map geometry | Typed decoding from the original parser numbering and retained captures. Two fresh docked E15 downloads through library 0.24.0 and bridge 0.12.1 decoded end to end on 2026-09-25, with Android stopped and a bridge restart between them. Static geometry matched the external source, see [receipt](research/E15_MAP_PROVISIONING.md#fresh-hardware-acquisition) | Coordinate and heading units and the meaning of `navPath.bin.stream` remain unconfirmed and display only. No live path-growth or mowing-update claim |
| Cleaning-path history                       | Library-owned accumulator merges successive decoded paths of one map identity and generation into one ordered history with the acquisition revision and receipt time per segment, synthetic fixtures, and the retained captures of 2026-09-10 replayed locally: the empty realtime placeholder changed nothing, a repeated history path merged unchanged, a stale replay was rejected and a changed grid started a new history without mixing, software evidence only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Growth of one path across acquisitions has synthetic coverage only, non-empty `realtime` paths were never observed on this transport and are handled structurally, and the merged history has not been compared with the app's displayed path on the owned device, recorded on [#178](https://github.com/keesmod/eufy-mega-client/issues/178)                                                                                                                                                                                                 |

The library leaves unconfirmed fields withheld and exposes raw snapshots
unchanged. Existing HA consumer behavior is outside this evidence. Map decoding is recorded in
...
in 0.24.0 supplied two fresh docked E15 downloads through bridge 0.12.1 on
2026-09-25, with Android stopped and a bridge restart between the downloads.
Both decoded and matched the external source's static geometry, with confirmed
cancellation and cleanup. This supplements the synthetic lifecycle coverage
and independent original-SDK evidence. Mowing updates, native-source outage
and map-source migration remain part of
[mower acceptance #8](https://github.com/keesmod/eufy-robomow-ha/issues/8#issuecomment-5835828129).
The Android source was restored and remains available for recovery.
