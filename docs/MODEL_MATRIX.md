...
| Commands                                    | Opt-in `start`, `pause` and `resume` over DP 1 and DP 2 confirmed in one owner-operated window on 2026-09-19: the written point echoed within a second, the confirmed DP 107 activity followed, and the app and the owner agreed on every step. `return` over DP 3 `switch_charge` ended `timed_out` without any report or effect | One window, one cycle per class from docked, mowing and paused. `return` is not honoured by firmware 6.9.28, the working return route is the app's Stop, Clear Progress and Charge over raw control points with uninterpreted values, and no library write of a raw point is established |
...
is not honoured. Remaining hardware work is a working return route, the
