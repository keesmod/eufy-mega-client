...
| Commands                                    | Opt-in `start`, `pause` and `resume` over DP 1 and DP 2 confirmed in one owner-operated window on 2026-09-19: the written point echoed within a second, the confirmed DP 107 activity followed, and the app and the owner agreed on every step. `return` over DP 3 `switch_charge` from `paused` ended `timed_out` without any report or effect. The opt-in `stop` class over DP 1 false and `return` from the stopped task are software only, synthetic tests without hardware evidence | One window, one cycle per class from docked, mowing and paused. `return` is not honoured by firmware 6.9.28 from `paused`, the working return route is the app's Stop, Clear Progress and Charge over raw control points with uninterpreted values, and no library write of a raw point is established. `stop` and `return` from the stopped task await the owner-operated window of [#173](https://github.com/keesmod/eufy-mega-client/issues/173) |
...
[#173](https://github.com/keesmod/eufy-mega-client/issues/173) are software
only until its owner-operated window. Remaining hardware work is that window,
the mowing-progress source, the undecoded DP 107 values and any dock,
