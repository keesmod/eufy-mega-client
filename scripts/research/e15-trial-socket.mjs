import net from 'node:net';
import tls from 'node:tls';
import { once } from 'node:events';

export function openSocket(owner, options, encrypted = false) {
  let socket, finishClose;
  const closed = new Promise((resolve) => {
    finishClose = resolve;
  });
  owner.own({
    close: () => {
      if (socket) socket.destroy();
      else finishClose(true);
      return closed;
    },
  });
  owner.signal.addEventListener('abort', () => socket?.destroy(), { once: true });
  // Queue bytes immediately, before a connect consumer can miss a fast response.
  const chunks = [];
  let length = 0,
    waiter,
    failure;
  const attach = () => {
    socket.on('error', () => {});
    socket.once('close', () => finishClose(true));
    socket.on('data', (b) => {
      length += b.length;
      if (length > 65536) {
        failure = new Error('socket-buffer-limit');
        socket.destroy();
      } else chunks.push(b);
      waiter?.();
      waiter = undefined;
    });
    socket.on('error', () => {
      failure = new Error('socket-failed');
      waiter?.();
      waiter = undefined;
    });
    socket.on('close', () => {
      failure ??= new Error('socket-closed');
      waiter?.();
      waiter = undefined;
    });
  };
  return {
    async connect() {
      if (socket || owner.signal.aborted) throw new Error('socket-already-consumed');
      socket = encrypted
        ? tls.connect({ ...options, servername: options.host, rejectUnauthorized: true })
        : new net.Socket();
      attach();
      if (encrypted) {
        await once(socket, 'secureConnect', { signal: owner.signal });
        if (!socket.authorized) throw new Error('tls-unverified');
      } else {
        socket.connect(options);
        await once(socket, 'connect', { signal: owner.signal });
      }
    },
    async read() {
      while (!chunks.length) {
        if (failure || owner.signal.aborted) throw failure ?? new Error('trial-aborted');
        await new Promise((resolve) => {
          waiter = resolve;
        });
      }
      const b = chunks.shift();
      length -= b.length;
      return b;
    },
    write(bytes) {
      if (failure || owner.signal.aborted || socket.destroyed) throw new Error('socket-closed');
      if (socket.writableLength + bytes.length > 65536) throw new Error('socket-write-limit');
      socket.write(bytes);
    },
  };
}
