...
export class PushClient extends TypedEmitter<PushClientEvents> {
  ...
  private initialize(): void {
    this.loggedIn = false;
    if (this.loginTimeout) { clearTimeout(this.loginTimeout); this.loginTimeout = undefined; }
  ...
  public connect(): void {
    ...
    this.loginTimeout = setTimeout(() => this.client?.destroy(), 20000);
    // For debugging purposes
    //this.client.enableTrace();

    this.client.on("secureConnect", () => this.onSocketConnect());
  ...
  private onSocketData(newData: Buffer): void {
    try { this.pushClientParser.handleData(newData); }
  ...
  private handleParsedMessage(message: Message): void {
    ...
    switch (message.tag) {
      ...
      case MessageTag.LoginResponse:
        ...
        if (message.object?.error) { this.client?.destroy(); break; }
        if (this.loginTimeout) { clearTimeout(this.loginTimeout); this.loginTimeout = undefined; }
        this.loggedIn = true;
        this.persistentIds = [];
