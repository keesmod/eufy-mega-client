...
export class PushClientParser extends TypedEmitter<PushClientParserEvents> {
  ...
  public resetState(): void {
    ...
    // Connection resets clear framing, not the owner's message subscription.
  }
  ...
  handleData(newData: Buffer): void {
    if (this.data.length + newData.length > 4 * 1024 * 1024) throw new Error('Push message too large');
  ...
  private onGotMessageSize(): void {
    ...
    try {
      ...
      if (this.messageSize < 0 || this.messageSize > 2 * 1024 * 1024) throw new Error('Invalid push message size');
    } catch (err) {
