...
export class PushNotificationService extends TypedEmitter<PushNotificationServiceEvents> {
  ...
  private async loadLibraries(): Promise<void> {
    ...
    this.got = got.extend({ timeout: { request: 15000 }, signal: this.lifetime.signal, followRedirect: false });
  }
  ...
  public getPersistentIds(): string[] {
    return this.pushClient?.getPersistentIds() ?? this.persistentIds;
  }
  ...
  private async _open(renew = false, forceNew = false): Promise<void> {
    ...
    if (this.credentials) {
      ...
      if (this.closed) { this.pushClient.close(); return; }
  ...
  public async open(): Promise<Credentials | undefined> {
    ...
    if (!this.connecting && !this.connected) {
      ...
      await this._open(false, false).catch((err) => {
        const error = ensureError(err);
  ...
  public close(): void {
    this.closed = true; this.lifetime.abort(); this.connected = false; this.connecting = false;
    this.resetRetryTimeout();
    this.clearCredentialsTimeout();
    this.pushClient?.close();
  }
