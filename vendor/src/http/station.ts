...
export class Station extends TypedEmitter<StationEvents> {
  ...
  protected constructor(
    ...
    this.p2pSession.on('encryption ready', mode => this.emit('encryption ready', this, mode));
    this.p2pSession.on('credential error', () => {
    ...
    this.p2pSession.on("download complete", (channel: number) => this.emit("download complete", this, channel));
    this.p2pSession.on(
      "livestream started",
      (channel: number, metadata: StreamMetadata, videoStream: Readable, audioStream: Readable) =>
  ...
  public updateRawProperty(type: number, value: string, source: SourceType): boolean {
    ...
    // A repeated device value is still fresh telemetry, unlike a command ACK.
    if (this.ready && parsedValue !== undefined) this.emit("parameter observed", this, type, parsedValue, source);
    if (
  ...
  public stopLivestream(device: Device, confirmEvenIfIdle = false): void {
    const commandData: CommandData = {
    ...
    if (!confirmEvenIfIdle && !this.isLiveStreaming(device)) {
  ...
  public databaseQueryByDate(
    ...
    if (![100, 500, 2000, 10000].includes(count)) throw new Error("Unsupported query count");
    const commandData: CommandData = {
