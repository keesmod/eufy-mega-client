...
export const decryptLockAESData = (hexkey: string, iv: string, data: Buffer): Buffer => {
  const key = Buffer.from(hexkey, "hex");
  const decipher = createDecipheriv(`aes-${key.length * 8}-cbc`, key, Buffer.from(iv, "hex"));
  return Buffer.concat([decipher.update(data), decipher.final()]);
};

export const encryptLockAESData = (hexkey: string, iv: string, data: Buffer): Buffer => {
  const key = Buffer.from(hexkey, "hex");
  const cipher = createCipheriv(`aes-${key.length * 8}-cbc`, key, Buffer.from(iv, "hex"));
  return Buffer.concat([cipher.update(data), cipher.final()]);
};
