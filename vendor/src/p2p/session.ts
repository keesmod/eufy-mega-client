...
import { CloudProvider } from "../http/api";
...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
  ...
  // Battery cameras can deliver the first AAC packet close to the former
  // 650 ms cutoff. Allow bounded transport jitter before declaring no audio;
  // streams with both media types still start as soon as those packets arrive.
  private readonly AUDIO_CODEC_ANALYZE_TIMEOUT = 3 * 1000;
  ...
  private socketClosing?: Promise<void>;
  private connectionClosing?: Promise<void>;
  private dskKey = "";
  private dskExpiration: Date | null = null;
  private deviceSNs: DeviceSerial = {};
  private api: CloudProvider;
  ...
  private initializeMessageState(datatype: P2PDataType, rsaKey: NodeRSA | null = null): void {
    // These callbacks read currentMessageState when they fire. Cancel them before
    // replacing their owner, including timers armed by late packets on an idle
    // stream. Otherwise an old transfer can stop media on a new connection.
    const previous = this.currentMessageState[datatype];
    this._clearTimeout(previous?.p2pStreamingTimeout);
    this._clearTimeout(previous?.waitForSeqNoTimeout);
    this._clearTimeout(previous?.waitForAudioData);
  ...
  private lookup(host?: string): void {
    ...
    if (this.connectionType !== P2PConnectionType.ONLY_LOCAL) {
      this.cloudLookup();
      this._clearLookup2Timeout();
      this.lookup2Timeout = setTimeout(() => this.cloudLookup2(), this.LOOKUP2_TIMEOUT);
    }
  ...
  public async connect(host?: string): Promise<void> {
    if (!this.connected && !this.connecting && this.rawStation.p2p_did !== undefined) {
      ...
      if (this.connectionType !== P2PConnectionType.ONLY_LOCAL) await this.renewDSKKey();
      if (!this.binded)
  ...
  private handleMsg(msg: Buffer, rinfo: RemoteInfo): void {
    if (hasHeader(msg, ResponseMessageType.LOCAL_LOOKUP_RESP)) {
      ...
      // The close event replaces the socket only after the old descriptor has
      // actually been released. Calling onClose directly leaks that socket.
      void this.closeSocket().catch((error) => this.onError(error));
      return;
  ...
  private waitForStreamData(dataType: P2PDataType, sendStopCommand = false): void {
    ...
    // A recovering camera can acknowledge START after the usual stall window.
    // Initial media has its own bounded deadline; received media arms the
    // shorter ongoing-data timeout without extending that stall protection.
    const timeout = sendStopCommand ? this.streamTimeouts.streamDataWait : this.streamTimeouts.streamStartWait;
    ...
    try {
      ...
      switch (message.commandId) {
        ...
        case CommandType.CMD_DOWNLOAD_FINISH:
          ...
          this.emit("download complete", this.currentMessageState[P2PDataType.BINARY].p2pStreamChannel);
          this.endStream(P2PDataType.BINARY);
          break;
        ...
        case CommandType.CMD_GATEWAYINFO:
          ...
          // Credential selection belongs to the caller. Never fetch a legacy API
          // or silently downgrade when a supplied cipher fails.
          this.api.getCommandCredentials(this.rawStation.station_sn, cipherID, this.rawStation.member.admin_user_id)
            .then(credentials => {
              if (!this.connected || this.terminating) return;
              if (credentials.mode === 'lan-derived') {
                if (this.connectionType !== P2PConnectionType.ONLY_LOCAL) throw new Error('LAN credentials require local transport');
                ...
                const rsa = getRSAPrivateKey(credentials.cipher.private_key, this.enableEmbeddedPKCS1Support);
                this.p2pKey = rsa.decrypt(encryptedKey);
                this.encryption = EncryptionType.LEVEL_2;
              }
              if (this.p2pKey.length !== 16) throw new Error('Invalid command key');
              this._clearTimeout(this.messageStates.get(message.seqNo)?.timeout);
              this.p2pSeqMapping.forEach((value, key) => { if (value === message.seqNo) this.p2pSeqMapping.delete(key); });
              this.p2pDataSeqNumber--;
              this.messageStates.delete(message.seqNo);
              this.emit('encryption ready', credentials.mode);
              this.sendQueuedMessage();
            })
            .catch(() => { this.emit('credential error'); void this.close(); });
          break;
        default:
  ...
  public close(): Promise<void> {
    // Download cancellation and its owner can both request cleanup. Sharing
    // the entire operation keeps one END send from racing socket disposal by
    // another close, which can otherwise strand a pending send callback.
    if (!this.connectionClosing)
      this.connectionClosing = this.closeConnection().finally(() => {
        this.connectionClosing = undefined;
      });
  ...
  private async closeConnection(): Promise<void> {
    ...
    // A new connection must use a new UDP endpoint so delayed END/media packets
    // from the old device session cannot tear down or corrupt the next one.
    await this.closeSocket();
  }

  private closeSocket(): Promise<void> {
    if (this.socketClosing) return this.socketClosing;
    if (!this.binded) return Promise.resolve();
    const socket = this.socket;
    this.socketClosing = new Promise<void>((resolve) => socket.close(() => resolve()))
      .finally(() => { this.socketClosing = undefined; });
  ...
  /** Final disposal closes UDP sockets and all timers; close() alone permits reconnection. */
  public async dispose(): Promise<void> {
    await this.close();
    this._clearLookup2Timeout(); this._clearLookup2RetryTimeout();
    this._clearKeepaliveTimeout(); this._clearESDDisconnectTimeout();
    this.socket.removeAllListeners();
    if (this.binded) await new Promise<void>(resolve => this.socket.close(() => resolve()));
    this.binded = false;
  ...
  private async getDSKKeys(): Promise<void> {
    const key = await this.api.getP2pKey(this.rawStation.station_sn);
    this.dskKey = key.key;
    this.dskExpiration = new Date(key.expiresAt * 1000);
  }
