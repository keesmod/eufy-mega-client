import type { RelayToken } from './types.js';
// Independently evidenced codecs promoted from #78/#49. See docs/MAP_ACQUISITION.md.
...
const mac = (algorithm: string, key: Buffer, data: Buffer | string) =>
  createHmac(algorithm, key).update(data).digest();
export function cbcEncrypt(key: Buffer, iv: Buffer, plain: Buffer) {
  if (key.length !== 16 || iv.length !== 16 || plain.length > 1300) fail();
  const cipher = createCipheriv('aes-128-cbc', key, iv);
  return Buffer.concat([iv, cipher.update(plain), cipher.final()]);
}
export function cbcDecrypt(key: Buffer, bytes: Buffer) {
...
function tlv(type: number, value: Buffer) {
...
function root(type: number, body: Buffer) {
...
export function attributes(bytes: Buffer, expectedType: number) {
  if (
    ...
    fail();
  const fields = new Map<number, { value: Buffer; offset: number }>();
...
export function decodeHandshake(
  ...
  const f = (k: number) => fields.get(k)!.value;
  const key = Buffer.from(token.credential).subarray(0, 16);
  if (
    f(1).length !== 2 ||
...
export function signature(token: RelayToken, account: string, suffix: string) {
...
export function dataRecord(key: Buffer, kcp: Buffer) {
  return root(0xf600, tlv(7, Buffer.concat([kcp, mac('sha1', key, kcp)])));
}
export function decodeData(key: Buffer, record: Buffer) {
...
export function authorization(credential: string, request = 0) {
...
export function versionRequest(request: number) {
...
export function versionResponse(bytes: Buffer, request: number) {
  if (
    bytes.length !== 24 ||
