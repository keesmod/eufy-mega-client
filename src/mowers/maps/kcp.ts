// Independently authored bounded KCP wire subset for the evidenced reliable TCP route.
...
export class MapKcp {
  ...
  #sendNext = 0;
  #receiveNext = 0;
  #pending = new Map<number, { fragment: number; payload: Buffer }>();
  #fragments: Buffer[] = [];
  #remaining?: number;
  #closed = false;
  #messages = 0;
  ...
  send(payload: Buffer, now: number) {
    if (
      this.#closed ||
