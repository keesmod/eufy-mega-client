...
import { Camera, SoloCamera, BatteryDoorbellCamera, Station } from '../dist/vendor/http/index.js';
...
for (const [model, type] of models) {
  test(`${model}/${type} selects SoloCamera, keeps actual owner/channel and observed state`, async () => {
    ...
    await fixture([owner(), owner('T8030_SECOND'), raw], async (t, inventory) => {
      ...
      assert.equal(sdk?.constructor, SoloCamera);
  ...
  test(`${model}/${type} correlates device and push detections, rejects wrong owners and rings`, async () => {
    ...
    await fixture([owner(), owner('T8030_WRONG'), raw], async (t) => {
      ...
      try {
        ...
        // Local detection, native SoloCam payload, HB3 companion and replay.
...
test('SoloCam state retains unknowns, accepts observed zero and follows owner/channel changes', async () => {
  for (const value of [undefined, null, '', '12bad', ' ', true, -1, 101, Infinity]) {
    const raw = camera('T8134', 63, { params: params(value, value), main_sw_version: '' });
  ...
  const raw = camera('T8134', 63, { params: params('0', '0') });
...
test('unsupported SoloCam variants, owners and initialization failures preserve other families', async () => {
  await fixture(
    [
      owner(),
      camera('T8124V', 62),
      camera('T8130', 63),
      camera('T8122', 60, { parent_sn: 'OLD_OWNER' }),
      camera('T8123', 61, { params: {} }),
      camera('T8134', 63),
      camera('T8172', 89),
      camera('T8213', 91),
    ],
    async (t, inventory) => {
      ...
      assert.throws(() => t.device('T8123_FIXTURE'), { code: 'device_initialization_failed' });
      assert.throws(() => t.device('T8122_FIXTURE'), { code: 'unsupported_station' });
      assert.equal(t.cameras.get('T8134_FIXTURE').constructor, SoloCamera);
...
test('public SoloCam inventory and observed-state API preserve existing identities', async () => {
  const rows = [
    owner(),
