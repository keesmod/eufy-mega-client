...
test('the E15 defaults report evidenced battery and network units without inferring activity', () => {
  ...
  // A full battery, an inactive task, an unverified status or map-save percentage is not
  // current activity or mowing progress. These independently invented values stay raw.
  const unsupported = decode({ ...e15Dps, 1: false, 5: 'charge_done', 8: 100, 118: 100 });
  assert.deepEqual(unsupported.status, { state: 'unconfirmed' });
...
test('the public local session returns the confirmed E15 fields and releases its connection', async (t) => {
  ...
  assert.deepEqual(telemetry.status, { state: 'unconfirmed' });
