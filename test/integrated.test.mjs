...
import { DoorbellCamera, BatteryDoorbellCamera, SmartDrop, DeviceProperties, PropertyName } from '../dist/vendor/http/index.js';
...
for (const p of integratedMedia) {
  test(`${p.model}: camera-only adapter, exact owner/channel and observed state`, async () => {
    ...
    try {
      ...
      assert.equal(sdk.constructor, p.type === 90 ? SmartDrop : p.type === 55 ? DoorbellCamera : BatteryDoorbellCamera);
  ...
  test(`${p.model}: native/H3 person events deduplicate and non-camera pushes are ignored`, async () => {
    const f = await mediaFixture(p), received = [];
    f.station.processPushNotification = () => {};
    f.station.hasCommand = () => false;
    const events = new EventTransport({}, (id) => f.transport.supportsEvent(id, 'notification'),
      (message) => f.transport.processPush(message), (message) => f.transport.acceptPush(message));
    f.transport.on('detection', (d) => events.device(d.id, d.type, d.name, d.stranger));
    events.on('event', (event) => received.push(event));
    try {
      const message = { device_sn: f.camera.device_sn, station_sn: f.camera.parent_sn,
        type: p.type, event_type: 3102, event_time: Date.now(), unique_id: 'synthetic-person',
      ...
      assert.equal(f.counts.starts, 0);
    } finally { await events.close(); await f.close(); }
  });
  ...
  test(`${p.model}: identity and unknown observations survive unavailable owners`, async () => {
    for (const parent of ['', `${p.model}_SYNTHETIC`, 'UNVERIFIED_BRIDGE']) {
      ...
      for (const value of ['', 'bad', ' ', true, -1, 101]) {
        ...
        assert.equal(result.issues[0].code, parent === 'UNVERIFIED_BRIDGE' ? 'unsupported_station' : 'standalone_transport_unverified');
      }
    }
  });
}
...
test('unproven integrated models stay unadmitted and do not fail a known camera', () => {
  ...
  const unknown = [['T8531', 189], ['E85V0', 203], ['T8530V', 55], ['UNKNOWN_GUN', 101], ['UNKNOWN_SNAIL', 102]];
  const result = discover([...rows, ...unknown.map(([device_model, device_type]) => ({
    ...camera, device_sn: device_model + '_FIXTURE', device_model, device_type,
  })), { ...camera, device_sn: 'MISMATCH_FIXTURE', device_type: 90 }]).result;
  ...
  for (const name of [PropertyName.DeviceMotionDetected, PropertyName.DevicePersonDetected, PropertyName.DeviceRinging])
...
test('public integrated camera API exposes no lock, lid or credential controls', async () => {
  const rows = integratedMedia.flatMap((p, index) => {
    const data = inventory(p); data.camera.device_channel = index;
    return index === 0 ? data.rows : [data.camera];
  });
  const cloud = cloudFixture({ inventory: rows }), client = new EufyMegaClient(cloud.options);
  try {
