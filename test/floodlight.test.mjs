...
const camera = (model, type, extra = {}) => ({
  ...
  device_type: type,
  device_channel: 2,
  device_name: 'Synthetic Floodlight',
...
for (const [model, type] of models) {
  test(`${model}/${type} selects FloodlightCamera, keeps actual owner/channel and observed state`, async () => {
    ...
    await fixture([owner(), owner('T8030_SECOND'), raw], async (t, inventory) => {
      ...
      assert.equal(sdk?.constructor, FloodlightCamera);
  ...
  test(`${model}/${type} correlates device and push detections, rejects wrong owners and rings`, async () => {
    ...
    await fixture([owner(), owner('T8030_WRONG'), raw], async (t) => {
      ...
      try {
        ...
        // Local detection, native Floodlight payload, HB3 companion and replay.
...
test('Floodlight state leaves absent properties unknown and follows owner/channel changes', async () => {
  for (const value of [undefined, null, '', '12bad', ' ', true, -1, 101, Infinity]) {
    const raw = camera('T8425', 47, { params: params(value, value), main_sw_version: '' });
  ...
  const raw = camera('T8425', 47, { params: params('0', '0') });
...
test('unsupported Floodlight models, owners and initialization failures preserve other families', async () => {
  await fixture(
    [
      owner(),
      ...[
        ...
        ['T8424', 39],
        ['T8425', 87],
      ].map(([model, type]) => camera(model, type, { device_sn: model + '_' + type })),
      camera('T8425', 47, { parent_sn: 'OLD_OWNER' }),
      camera('T8426', 87, { params: {} }),
      camera('T8172', 89),
      camera('T8213', 91),
    ],
    async (t, inventory) => {
...
test('public Floodlight inventory and observed-state API preserve existing identities', async () => {
  const rows = [
    owner(),
